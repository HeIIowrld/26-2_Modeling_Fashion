...
무신사 검색 카드의 상품 이미지가 준비된 경우 검색된 상의×하의 전체 조합도 같은 마스크로
자동 순차 생성한다. 한쪽 카테고리만 있으면 각 상품을 단독 생성하고, 조합별 실패는 격리하며
기존 결과 캐시를 재사용한다. 신발은 전용 마스크·생성 경로가 없어 지원 대상으로 표시하지 않는다.
