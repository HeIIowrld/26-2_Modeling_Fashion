...
- 웹의 변경 범위는 상의·하의·신발 체크박스다. 선택한 카테고리마다 무신사 상품을 최대 3개씩 검색한다(모두 선택하면 최대 9개).
- 예산은 **상품 한 개당** 적용한다. 결과가 부족하거나 검색이 실패하면 다른 카테고리로 대체하지 않는다.
- 신발 키워드는 목적·스타일·색상·활동 조건을 사용한다. 신발에는 체형 보정 규칙을 적용하지 않는다.
- 신발 인식은 독립 FashionSigLIP 헤드를 준비한 상태다. 데이터 학습 전에는 `학습 준비 중`으로 표시하고 입력 조건으로 추천한다.
- 신발 가상 피팅은 미지원이며, 기존 상·하의 2×3 점수표는 그대로 유지한다.
- 학습 연결 방법: [신발 헤드 안내](ai_fashion_recommender/SHOE_HEAD.md).

추천 입력 예: `UserProfile(change_categories=["top", "shoes"])`.
기존 노트북의 `change_scope`도 호환되며, `change_categories`가 있으면 이를 우선한다.
