...
지금은 모델이 없어 **"준비 중"** 안내만 나오고, 모델이 준비되면 다음 두 가지만 하면 화면이 바로 동작합니다.

1. `virtual_tryon.py`의 `VirtualTryOnAdapter._synthesize()`에 체크포인트를 연결합니다.
   `(person_image, recommendation, output_path)`를 받아 결과를 저장하고 그 경로를 돌려주면 됩니다.
