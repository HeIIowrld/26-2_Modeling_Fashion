...
- 통합 규칙 55개 중 48개가 구현되어 있습니다. 이 중 36개는 후보 필터·순위 점수에, 나머지는 분석 신뢰도 안전장치와 신발·액세서리 스타일링 안내에 사용됩니다. 상세 날씨와 보유 옷처럼 선택 입력이 필요한 규칙은 값이 있을 때만 실제 추천에 적용됩니다.
...
추천 엔진의 체형 규칙 coverage는 다음과 같습니다.

| 체형 | 추천 규칙 | 주요 keyword |
| --- | --- | --- |
| 역삼각체형 | R-BOD-02 | 레귤러, 정돈된 핏, 스트레이트, 세미와이드, 와이드 |
| 삼각체형 | R-BOD-01, R-BOD-06 | 어깨 구조, 넥라인 포인트, 레귤러, 여유핏, 스트레이트, 세미와이드 |
| 사각체형 | R-BOD-03 | 허리 기준점, 세미핏, 스트레이트, 세미와이드 |
| 모래시계체형 | R-BOD-03 | 허리 기준점, 세미핏, 스트레이트, 세미와이드 |
| 둥근체형 | R-BOD-07 | V넥, 넥라인 포인트, 레귤러, 여유핏, 기본 기장, 스트레이트, 세미와이드, 풀렝스 |
| 마름모꼴체형 | R-BOD-08 | 어깨 구조, 넥라인 포인트, 레귤러, 여유핏, 스트레이트, 세미와이드, 풀렝스 |

모래시계체형은 어깨와 엉덩이 폭이 비슷하다는 기존 정의에 따라 사각체형과 같은
R-BOD-03을 사용합니다. 둥근체형과 마름모꼴체형은 각각 R-BOD-07과 R-BOD-08을
사용하며, 둘레 입력과 confidence 0.65 이상일 때만 체형 keyword가 생성됩니다.
...
현재 배포 중인 `models/fashion_attribute_heads_augmented.pt`는 의류 crop **22,341개**로 학습한 데이터 보강 버전입니다.
초기 baseline(crop 4,789개)은 `models/fashion_attribute_heads.pt`에 rollback용으로 보관되어 있습니다.
...
현재 22,341-crop 체크포인트의 17개 학습 헤드는 그대로 유지하고, 추론 단계에 착용 상태 분석을
추가했습니다. 체크포인트 스키마를 즉시 바꾸면 기존 가중치를 읽지 못하기 때문에 새 항목은 우선
별도 분석 계층으로 동작합니다.

- `sleeve_length`: 학습 헤드가 판단한 설계상 소매 길이
- `visible_sleeve_length`: 마스크에서 현재 사진에 보이는 길이
- `sleeve_state`: 정상 / 걷음 가능성 높음 / 좌우 비대칭 / 가림 또는 불확실 / 판단 보류
- `layering_state`: 단일 상의 / 레이어드 / 레이어드 가능성 / 판단 보류
- `upper_items`, `inner_category`, `outer_category`: 복수 상의와 안쪽·바깥쪽 추정

레이어드는 상의 전체 crop과 MediaPipe 어깨 좌표로 만든 목·윗가슴 ROI의 FashionSigLIP
상대점수, category/collar/neckline/material 사이의 모순을 함께 봅니다. 니트 계열 상의와
셔츠형 칼라가 동시에 관찰되면 기존의 `티셔츠 + 폴로 칼라 → 폴로 셔츠` 강제 보정을 적용하지
않습니다. 이 단계는 인스턴스 분할 모델이 아니므로 안쪽 옷이 거의 보이지 않을 때는 확정하지
않고 가능성 또는 판단 보류로 출력합니다.

정식 `sleeve_state`·`layering_state` 학습 헤드는 해당 라벨 데이터가 준비된 뒤 체크포인트
버전을 올려 추가해야 합니다. 그 전까지 기존 17개 속성 성능을 보존하는 호환 계층입니다.
...
추천 엔진은 그대로 동작합니다.** 지금 데이터는 규칙 48개가 전부 발동될 수 있도록 값을 일부러 흩어 놓았습니다.
