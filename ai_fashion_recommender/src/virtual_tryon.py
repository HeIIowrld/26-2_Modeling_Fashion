...
class VirtualTryOnAdapter:
    ...
    def synthesize(
        ...
        _synthesize()를 직접 부르지 않고 generate()로 넘긴다. CatVTONTryOn처럼
        generate()를 재정의한 구현체가 이 문으로도 동작해야 하기 때문이다.
        """
        if not self.enabled:
            raise TryOnNotReady(self.NOT_READY_REASON)
        return self.generate(person_image, recommendation, output_path, context)
