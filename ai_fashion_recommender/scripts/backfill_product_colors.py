"""이미 만들어 둔 카탈로그의 색만 무신사 옵션으로 다시 채운다.
...
usage:
    python scripts/backfill_product_colors.py                     # 제자리 갱신
...
def main() -> int:
    ...
    changed = multi = emptied = 0
    for row, names in zip(targets, fetched):
        if not names:
            continue  # 색 옵션이 없는 단색 상품. 기존 값을 지우지 않는다.
        ...
        # 표에 없는 이름이면 색을 지어내지 않는다. 사진 색 감사가 채울 수 있다.
        row["color"] = palettes[0] if palettes else ""
        changed += row["color"] != before
        multi += len(palettes) > 1
        emptied += not palettes
    elapsed = time.monotonic() - started

    print(f"대상 {len(targets)}개 / 색 옵션 확보 {sum(bool(n) for n in fetched)}개 "
          f"({elapsed:.1f}초, 상품당 {elapsed / max(1, len(targets)):.3f}초)")
    print(f"  대표 색이 바뀐 상품 {changed}  여러 색으로 채워진 상품 {multi}  "
          f"표에 없어 비운 상품 {emptied}")
