...
function renderResult(result) {
  ...
  resetPrivacyBar();
  $("result-lede").textContent = shoppingResults.length
    ? "사진과 입력 조건에서 만든 키워드로 무신사 상품을 실시간 검색했습니다."
    : "실시간 상품을 찾지 못했어요. 조건을 조금 넓혀 다시 시도해주세요.";

  renderCurrentOutfitEvaluation(result.current_outfit_evaluation);
  renderCurrentOutfit(result);
  renderBodyStats(result.pose);
  renderShoppingProducts(shoppingResults);
  if (state.tryon.available && shoppingResults.some((product) => product.tryon_available)) {
