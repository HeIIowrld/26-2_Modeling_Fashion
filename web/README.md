...
개별 무신사 상품 선택 근거도 상품명과 매칭된 키워드 중 적용 규칙·입력 출처·분석 신뢰도가
확인된 근거만 사용합니다. 상품명은 카드에 그대로 표시하고 내부 검증에도 사용하지만,
`상품 추천 근거` 문장에서는 상품명 자체나 상품명과 일치한다는 내용을 근거로 말하지 않습니다.
근거가 없으면 설명을 임의로 만들지 않습니다. 발표 서버에서
...
응답에는 실제 사용한 근거 ID를 함께 반환해야 하며, 검증된 키워드를 포함하지 않거나
`상품 추천 근거`에서 상품명·제품명·타이틀을 근거로 언급하거나 허용되지 않은 근거 ID를 사용하면 폐기합니다. 예산·가격·할인·가성비·사이즈는
