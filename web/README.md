...
우선 요청할 수도 있고, 이미 생성된 조합은 캐시를 재사용합니다. 상·하의는 CatVTON으로,
신발은 별도 참조 이미지 기반 모델로 발 주변만 합성합니다. 신발 모델이 준비되고 양쪽 발이
보이는 경우에만 신발 합성을 지원합니다. 모델 설정과 공개 화면의 별도 배포 상태는
[`gpu_server/SHOES_VTON.md`](../gpu_server/SHOES_VTON.md)를 확인합니다.
...
직접 옷을 권하는 옷가게 점원처럼 사용자 조건과 실제 검색 키워드를 바탕으로 자연스러운
한 문장의 추천 이유를 만듭니다. 고정 문장 형식은 강제하지 않지만 예산·가격·할인·가성비
등 금액 관련 내용은 넣지 않습니다. 신발 추천에는 체형이나 다리 길이를 근거로 사용하지
않으며, 금액을 언급한 문장은 규칙 기반 문장으로 대체합니다.
...
조건 화면의 선택 항목에서 `웜톤` 또는 `쿨톤`을 고르면 일반 색상명으로 무신사 후보를
검색한 뒤 상품 이미지를 분석해 선택한 톤과 일치하는 상품만 남깁니다. 피부톤을 선택하지
않으면 기존 검색 결과를 그대로 사용합니다. 이미지 판정에 실패하거나 신뢰도가 낮은
상품은 특정 톤으로 추측하지 않습니다.

데님은 일반 의류와 별도로 워싱과 디테일을 봅니다. 빈티지 중청·티스테인·브라운 워싱,
골드/브론즈 버튼과 오렌지/브라운 스티치는 웜 근거로, 아이스 블루·딥 인디고·그레이시
블루, 실버 부자재와 화이트/그레이 스티치는 쿨 근거로 사용합니다. 썸네일에서 작은
부자재를 확실하게 확인할 수 없으면 상품명 단서와 전체 워싱 색을 우선합니다.
